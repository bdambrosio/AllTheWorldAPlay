...
class PandasModel(QAbstractTableModel):
    ...
    def removeRow(self, row, parent=None):
        global section_indexIDMap, section_library_df, section_indexIDMap
        self.beginRemoveRows(QModelIndex(), row, row)

        df_row = self._data.loc[self._data.index[row]]
        section_faiss_id = df_row['faiss_id']
        print(f'Removing section {section_faiss_id}')

        #first find and remove all sections from section_faiss and section_library_df:
        section_library_rows = section_library_df[section_library_df['section_id'].asType(str) == str(id)]
        if section_library_rows is None:
            return
...
model = PandasModel(section_library_df, table_view)
...
def find_by_id(faiss_id):
    try:
        ...
        matching_rows = section_library_df[section_library_df['faiss_id'].astype(str) == str(faiss_id)]

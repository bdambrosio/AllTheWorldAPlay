...
class ExLlamaV2BaseGenerator:
    ...
    def generate_simple(
        self,
        prompt: str or list, # type: ignore
        gen_settings: ExLlamaV2Sampler.Settings,
        num_tokens: int,
        seed: int or None = None, # type: ignore
        token_healing: bool = False,
        encode_special_tokens: bool = False,
        decode_special_tokens: bool = False,
        loras: ExLlamaV2Lora or list[ExLlamaV2Lora] | None = None, # type: ignore
        stop_token: int or None = -1, # type: ignore
        add_bos: bool = False,
        ...
        for i in range(num_tokens):
            ...
            if stop_seq is not None and len(stop_seq) > 0 and len(stop_seq[0])> 0:
                # Check if the sequence is long enough to contain the stop sequence
                if self.sequence_ids.shape[1] >= len(stop_seq[0]):
                    # Get the end of the tensor
                    stop_len = len(stop_seq[0])
                    end_of_sequence = self.sequence_ids[0, -stop_len:]
                    # Check if it matches the stop sequence
                    if torch.all(end_of_sequence == stop_seq[0]).item():
                        break
    ...
    def _gen_begin_base(self,
                        ...
                        loras: ExLlamaV2Lora or list[ExLlamaV2Lora] or None = None, # type: ignore
